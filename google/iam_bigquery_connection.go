...
func BigqueryConnectionConnectionIamUpdaterProducer(d TerraformResourceData, config *Config) (ResourceIamUpdater, error) {
	...
	if v, ok := d.GetOk("connection_id"); ok {
		values["connection_id"] = v.(string)
	}

	// We may have gotten either a long or short name, so attempt to parse long name if possible
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/connections/(?P<connection_id>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<connection_id>[^/]+)", "(?P<location>[^/]+)/(?P<connection_id>[^/]+)", "(?P<connection_id>[^/]+)"}, d, config, d.Get("connection_id").(string))
	if err != nil {
		return nil, err
	}
	...
	u := &BigqueryConnectionConnectionIamUpdater{
	...
	if err := d.Set("connection_id", u.GetResourceId()); err != nil {
		return nil, fmt.Errorf("Error setting connection_id: %s", err)
	}
...
func BigqueryConnectionConnectionIdParseFunc(d *schema.ResourceData, config *Config) error {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/connections/(?P<connection_id>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<connection_id>[^/]+)", "(?P<location>[^/]+)/(?P<connection_id>[^/]+)", "(?P<connection_id>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *BigqueryConnectionConnectionIamUpdater) GetResourceIamPolicy() (*cloudresourcemanager.Policy, error) {
	url, err := u.qualifyConnectionUrl("getIamPolicy")
	if err != nil {
		return nil, err
	}
...
func (u *BigqueryConnectionConnectionIamUpdater) SetResourceIamPolicy(policy *cloudresourcemanager.Policy) error {
	...
	url, err := u.qualifyConnectionUrl("setIamPolicy")
	if err != nil {
		return err
	}
	project, err := getProject(u.d, u.Config)
...
func (u *BigqueryConnectionConnectionIamUpdater) DescribeResource() string {
	return fmt.Sprintf("bigqueryconnection connection %q", u.GetResourceId())
}
