...
func resourceComputeRegionNetworkFirewallPolicyCreate(d *schema.ResourceData, meta interface{}) error {
	...
	id, err := replaceVarsForId(d, config, "projects/{{project}}/regions/{{region}}/firewallPolicies/{{name}}")
	if err != nil {
...
func resourceComputeRegionNetworkFirewallPolicyRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		resourceName := fmt.Sprintf("ComputeRegionNetworkFirewallPolicy %q", d.Id())
	...
	if err = d.Set("region_network_firewall_policy_id", res.Id); err != nil {
		return fmt.Errorf("error setting region_network_firewall_policy_id in state: %s", err)
	}
