...
func CloudIotDeviceRegistryIamUpdaterProducer(d TerraformResourceData, config *Config) (ResourceIamUpdater, error) {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/registries/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Get("name").(string))
	if err != nil {
		return nil, err
	}
	...
	u := &CloudIotDeviceRegistryIamUpdater{
...
func CloudIotDeviceRegistryIdParseFunc(d *schema.ResourceData, config *Config) error {
	...
	m, err := getImportIdQualifiers([]string{"projects/(?P<project>[^/]+)/locations/(?P<location>[^/]+)/registries/(?P<name>[^/]+)", "(?P<project>[^/]+)/(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<location>[^/]+)/(?P<name>[^/]+)", "(?P<name>[^/]+)"}, d, config, d.Id())
	if err != nil {
		return err
	}
...
func (u *CloudIotDeviceRegistryIamUpdater) GetResourceIamPolicy() (*cloudresourcemanager.Policy, error) {
	url, err := u.qualifyDeviceRegistryUrl("getIamPolicy")
	if err != nil {
		return nil, err
	}
...
func (u *CloudIotDeviceRegistryIamUpdater) SetResourceIamPolicy(policy *cloudresourcemanager.Policy) error {
	...
	url, err := u.qualifyDeviceRegistryUrl("setIamPolicy")
	if err != nil {
		return err
	}
	project, err := getProject(u.d, u.Config)
...
func (u *CloudIotDeviceRegistryIamUpdater) GetResourceId() string {
	return fmt.Sprintf("projects/%s/locations/%s/registries/%s", u.project, u.region, u.name)
}

func (u *CloudIotDeviceRegistryIamUpdater) GetMutexKey() string {
	return fmt.Sprintf("iam-cloudiot-deviceregistry-%s", u.GetResourceId())
}

func (u *CloudIotDeviceRegistryIamUpdater) DescribeResource() string {
	return fmt.Sprintf("cloudiot deviceregistry %q", u.GetResourceId())
}
